...
class OrderService {
  ...
  Future<List<Map<String, dynamic>>> getSellerOrders() async {
    ...
    final snapshot = await _firestore.collection(AppConstants.ordersCollection)
  ...
  Future<Map<String, dynamic>?> getOrder(String orderId) async {
    final doc = await _firestore.collection(AppConstants.ordersCollection)
        .doc(orderId)
        .get();
  ...
  Future<void> updateOrderStatus(String orderId, String status) async {
    ...
    await _firestore.collection(AppConstants.ordersCollection)
        .doc(orderId)
        .update({
          'status': status,
  ...
  Future<Map<String, dynamic>> getOrderStatistics() async {
    ...
    final pendingCount = allOrders.where((order) => order['status'] == AppConstants.orderStatusPending).length;
    final preparingCount = allOrders.where((order) => order['status'] == AppConstants.orderStatusPreparing).length;
    final deliveringCount = allOrders.where((order) => order['status'] == AppConstants.orderStatusDelivering).length;
    final deliveredCount = allOrders.where((order) => order['status'] == AppConstants.orderStatusDelivered).length;
    final cancelledCount = allOrders.where((order) => order['status'] == AppConstants.orderStatusCancelled).length;
  ...
  Stream<QuerySnapshot> listenToNewOrders() {
    ...
    return authService.getCurrentUserId().then((sellerId) {
      if (sellerId != null) {
        return _firestore.collection(AppConstants.ordersCollection)
            .where('sellerId', isEqualTo: sellerId)
...
// مزود طلب محدد
final orderProvider = FutureProvider.family<Map<String, dynamic>?, String>((ref, orderId) async {
  final orderService = ref.watch(orderServiceProvider);
  return await orderService.getOrder(orderId);
});

// مزود الطلبات الجديدة
final newOrdersProvider = FutureProvider<List<Map<String, dynamic>>>((ref) async {
  final orderService = ref.watch(orderServiceProvider);
