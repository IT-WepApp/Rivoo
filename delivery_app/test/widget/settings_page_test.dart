...
void main() {
  group('SettingsPage Tests', () {
    ...
    final router = GoRouter(
      routes: [
        ...
        GoRoute(
          ...
          builder: (context, state) => const Scaffold(body: Text('Language Page')),
        ),
      ],
    );

    testWidgets('يجب أن تعرض صفحة الإعدادات العناصر الأساسية', (WidgetTester tester) async {
      ...
      expect(find.byIcon(Icons.info_outline), findsOneWidget); // أيقونة معلومات التطبيق
    ...
    testWidgets('يجب أن يتم الانتقال إلى صفحة اختيار اللغة عند النقر على خيار اللغة', (WidgetTester tester) async {
      // بناء الواجهة للاختبار
      await tester.pumpWidget(
        ProviderScope(
          child: MaterialApp.router(
    ...
    testWidgets('يجب أن يتم عرض مربع حوار السمة عند النقر على خيار السمة', (WidgetTester tester) async {
      // بناء الواجهة للاختبار
      await tester.pumpWidget(
        ProviderScope(
          child: MaterialApp.router(
      ...
      expect(find.byType(RadioListTile<ThemeMode>), findsNWidgets(3)); // ثلاثة خيارات للسمة
    });
  });
}
