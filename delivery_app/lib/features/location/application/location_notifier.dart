...
class LocationNotifier extends StateNotifier<AsyncValue<Position>> {
  ...
  Future<void> _initializeLocation() async {
    ...
    try {
      ...
      if (permission == LocationPermission.denied) {
        ...
        if (permission == LocationPermission.denied) {
          state = AsyncError('Location permissions are denied', StackTrace.current);
          return;
        }
      }
      
      if (permission == LocationPermission.deniedForever) {
        state = AsyncError('Location permissions are permanently denied', StackTrace.current);
        return;
      }
      ...
      log('Error initializing location', error: e, stackTrace: stackTrace, name: 'LocationNotifier');
  ...
  void startTrackingLocation() {
    ...
    _positionStreamSubscription = Geolocator.getPositionStream(locationSettings: locationSettings).listen(
      ...
      onError: (error, stackTrace) {
        log('Error tracking location', error: error, stackTrace: stackTrace, name: 'LocationNotifier');
         if (mounted) {
  ...
  Future<void> updateLocationOnce() async {
     ...
     try {
       ...
       log('Error getting single location update', error: e, stackTrace: stackTrace, name: 'LocationNotifier');
       state = AsyncError('Failed to get location: $e', stackTrace);
     }
  }
