...
class StoreDetailsPage extends ConsumerWidget { 
  ...
  static final storeDetailsProvider = FutureProvider.autoDispose.family<StoreModel?, String>((ref, storeId) {
      final storeService = ref.watch(storeServiceProvider);
      return storeService.getStoreDetails(storeId);
  });

  Future<void> _addToCart(WidgetRef ref, BuildContext context, Product product) async {
                 ...
                 slivers: <Widget>[
                   SliverAppBar(
                     ...
                     flexibleSpace: FlexibleSpaceBar(
                       background: storeData.imageUrl != null && storeData.imageUrl!.isNotEmpty
                           ? Image.network(
                               storeData.imageUrl!,
                               fit: BoxFit.cover,
                   ...
                   productsAsync.when(
                                    ...
                                    child: AppCard(
                                      child: ListTile(
                                        leading: product.imageUrl.isNotEmpty
                                            ...
                                            : const SizedBox(width: 60, height: 60, child: Icon(Icons.shopping_bag)),
                                        title: Text(product.name, style: Theme.of(context).textTheme.titleMedium),
                                        subtitle: Text('Price: \$${product.price.toStringAsFixed(2)}'), // Use directly
                                        trailing: AppButton( 
                                          onPressed: () => _addToCart(ref, context, product),
                                          text: 'Add', 
                                        ),
                                      ),
                                    ),
                      ...
                      error: (err, stack) => SliverFillRemaining(child: Center(child: Text('Error loading products: $err'))),
                   ),
                 ],
            );
         },
         loading: () => const Center(child: CircularProgressIndicator()),
         error: (err, stack) => Center(child: Text('Error loading store details: $err')),
      )
    );
  }
}
