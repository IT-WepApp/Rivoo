...
/// حارس المصادقة للتحقق من صلاحيات الوصول
class AuthGuard {
  /// التحقق من حالة تسجيل الدخول
  static Future<String?> guardAuth(BuildContext context, GoRouterState state, WidgetRef ref) async {
  ...
  static Future<String?> guardUnauth(BuildContext context, GoRouterState state, WidgetRef ref) async {
    final authState = ref.read(authStateProvider);
  ...
  static Future<String?> guardRole(
    ...
    if (authState.user != null && allowedRoles.contains(authState.user!.role)) {
      // المستخدم لديه الصلاحيات المطلوبة، السماح بالوصول
      return null;
    }
    
    // المستخدم ليس لديه الصلاحيات المطلوبة، إعادة التوجيه إلى صفحة غير مصرح
    return '/unauthorized';
  }

  /// التحقق من صلاحيات المسؤول
  static Future<String?> guardAdmin(BuildContext context, GoRouterState state, WidgetRef ref) async {
    return guardRole(context, state, ref, [UserRole.admin]);
  }

  /// التحقق من صلاحيات العميل
  static Future<String?> guardCustomer(BuildContext context, GoRouterState state, WidgetRef ref) async {
    return guardRole(context, state, ref, [UserRole.customer]);
  }

  /// التحقق من صلاحيات السائق
  static Future<String?> guardDriver(BuildContext context, GoRouterState state, WidgetRef ref) async {
    return guardRole(context, state, ref, [UserRole.driver]);
  }

  /// التحقق من صلاحيات المسؤول أو العميل
  static Future<String?> guardAdminOrCustomer(BuildContext context, GoRouterState state, WidgetRef ref) async {
    return guardRole(context, state, ref, [UserRole.admin, UserRole.customer]);
  }

  /// التحقق من صلاحيات المسؤول أو السائق
  static Future<String?> guardAdminOrDriver(BuildContext context, GoRouterState state, WidgetRef ref) async {
