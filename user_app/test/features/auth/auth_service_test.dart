...
void main() {
  ...
  setUp(() {
    ...
    authService =
        AuthService(mockSecureStorage, mockFirebaseAuth, mockFirestore);
  ...
  group('AuthService Tests', () {
    test('signIn should save auth token and user data securely', () async {
      ...
      when(mockSecureStorage.saveAuthToken(any,
              expiryMinutes: anyNamed('expiryMinutes')))
      ...
      verify(mockSecureStorage.saveAuthToken(any,
    ...
    test('getUserRole should get role from secure storage first if available',
        () async {
      // ترتيب
      final userData = {
        'uid': 'test_user_id',
    ...
    test('getUserRole should get role from Firestore if not in secure storage',
        () async {
      // ترتيب
      ...
      verify(mockSecureStorage.getUserData()).called(1);
      verify(mockFirestore.collection('users').doc('test_user_id').get())
          .called(1);
    });

    test(
        'updateUserRole should update role in both Firestore and secure storage',
        () async {
      // ترتيب
      final userData = {
        'uid': 'test_user_id',
