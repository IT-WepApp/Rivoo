...
class UserListPage extends ConsumerStatefulWidget { // Changed to StatefulWidget
...
class _UserListPageState extends ConsumerState<UserListPage> {
  ...
  final List<String> _roleOptions = ['All', 'Admin', 'Seller', 'User', 'Delivery'];
 ...
 void dispose() {
  ...
  Widget build(BuildContext context) {
    ...
    ref.listen<AsyncValue<List<UserModel>>>(userManagementProvider, (previous, next) {
          ...
          Expanded(
            child: userListState.when(
              ...
              error: (error, stackTrace) => Center(
                child: Text('Failed to load users. Pull down to retry. Error: $error')
              ),
  ...
  Widget _buildUserList(List<UserModel> users, WidgetRef ref) {
    // Filter users locally
     final filteredUsers = users.where((user) {
      final roleMatch = _selectedRoleFilter == 'All' || (user.role.toLowerCase() == _selectedRoleFilter.toLowerCase());
    ...
    return RefreshIndicator(
        ...
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return UserListItem(
              user: user,
              availableRoles: _roleOptions.where((r) => r != 'All').toList(), // Pass actual roles
              onDelete: () {
                  _showConfirmationDialog(context, ref, user.id, 'delete');
                             },
              onEditRole: (newRole) {
                 if (newRole != user.role) {
                    _showConfirmationDialog(context, ref, user.id, 'editRole', newRole: newRole, currentUser: user);
                 }
              },
            );
          },
   ...
   void _showConfirmationDialog(BuildContext context, WidgetRef ref, String userId, String action, {String? newRole, UserModel? currentUser}) {
     ...
     switch (action) {
        case 'delete':
           ...
           onConfirm = () async {
              final success = await ref.read(userManagementProvider.notifier).deleteUser(userId);
         ...
         case 'editRole':
           ...
           onConfirm = () async {
             final success = await ref.read(userManagementProvider.notifier).editUser(currentUser.copyWith(role: newRole)); // Assuming copyWith exists
              if (!success && context.mounted) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
             style: ElevatedButton.styleFrom(backgroundColor: action == 'delete' ? Colors.red : Colors.green),
            onPressed: () {
               onConfirm();
              Navigator.pop(context); // Close the dialog
            },
            child: Text('Confirm', style: TextStyle(color: action == 'delete' ? Colors.white : null)),
          ),
        ],
      ),
    );
