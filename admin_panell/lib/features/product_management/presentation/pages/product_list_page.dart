...
class _ProductListPageState extends ConsumerState<ProductListPage> {
  ...
  final List<String> _statusOptions = ['All', 'Pending', 'Approved', 'Rejected']; // Assuming these statuses are used
  final List<String> _categoryOptions = ['All', 'Electronics', 'Clothing', 'Food']; // Placeholder - fetch dynamically if possible
 ...
 void dispose() {
  ...
  Widget build(BuildContext context) {
    ...
    final productsState = ref.watch(productManagementProvider);
     ...
     // Listen for errors
    ref.listen<AsyncValue<List<Product>>>(productManagementProvider, (previous, next) { // Changed ProductModel to Product
  ...
  Widget _buildProductList(List<Product> products, WidgetRef ref) { // Changed ProductModel to Product
    ...
    final filteredProducts = products.where((product) {
      ...
      final statusMatch = _selectedStatus == 'All'; // Implement actual status filtering based on model
      
      // Check Product model for categoryId field nullability (it's required)
      final categoryMatch = _selectedCategory == 'All'; // Implement category filtering based on actual categories
      ...
      final searchMatch = _searchText.isEmpty ||
                           ...
                           (product.description.toLowerCase().contains(_searchText)); // description is not nullable
    ...
    return RefreshIndicator(
        onRefresh: () => ref.read(productManagementProvider.notifier).fetchProducts(),
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            return ProductListItem(
              ...
              onApprove: () => ref.read(productManagementProvider.notifier).approveProduct(product.id), 
              onReject: () => ref.read(productManagementProvider.notifier).rejectProduct(product.id), 
              onEdit: () {
                 ScaffoldMessenger.of(context).showSnackBar(
                   SnackBar(content: Text('Edit action for ${product.name} (TODO)')),
                 );
              },
            );
          },
              ...
              child: Column(
                ...
                children: [
                  ...
                  Chip(
                    ...
                    label: const Text('Status Placeholder'), // Placeholder - Update based on actual status field
            ...
            Column(
               ...
               children: [
                 IconButton(
                   ...
                   onPressed: onApprove, // Add logic based on actual status field
                 ),
                 IconButton(
